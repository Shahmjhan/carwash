    ...
    <form method="post" action="{{ route('services.update', $service) }}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
            <div class="form-group">
                ...
                <input type="text" name="name" value="{{ $service->name }}" required autofocus style="padding:12px;border:1px solid #e5e7eb;border-radius:8px;width:100%;">
            </div>
            <div class="form-group">
        ...
        <div class="form-group">
            ...
            <textarea name="description" rows="3" style="padding:12px;border:1px solid #e5e7eb;border-radius:8px;width:100%;">{{ $service->description }}</textarea>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;">
            <div class="form-group">
                <label>Base Price (Rs.) *</label>
                <input type="number" name="base_price" step="0.01" min="0" value="{{ $service->base_price }}" required style="padding:12px;border:1px solid #e5e7eb;border-radius:8px;width:100%;">
            </div>
            <div class="form-group">
                <label>Labor Cost (Rs.)</label>
                <input type="number" name="labor_cost" step="0.01" min="0" value="{{ $service->labor_cost }}" style="padding:12px;border:1px solid #e5e7eb;border-radius:8px;width:100%;">
            </div>
            <div class="form-group">
                <label>Tax Rate (%)</label>
                <input type="number" name="tax_rate" step="0.01" min="0" max="100" value="{{ $service->tax_rate }}" style="padding:12px;border:1px solid #e5e7eb;border-radius:8px;width:100%;">
            </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
            <div class="form-group">
                <label>Duration (minutes)</label>
                <input type="number" name="duration_minutes" min="1" value="{{ $service->duration_minutes }}" style="padding:12px;border:1px solid #e5e7eb;border-radius:8px;width:100%;">
            </div>
            <div class="form-group">
                <label>Status</label>
                <div style="display:flex;align-items:center;gap:10px;padding:12px 0;">
                    <label style="position:relative;display:inline-block;width:44px;height:24px;">
                        <input type="checkbox" name="active" {{ $service->active ? 'checked' : '' }} style="opacity:0;width:0;height:0;">
                    ...
                    <span style="font-size:14px;color:#6b7280;">{{ $service->active ? 'Active' : 'Inactive' }}</span>
                </div>
            </div>
        </div>
        <div style="margin-top:30px;">
            <button type="submit" class="primary" style="background:#3b82f6;padding:12px 24px;border-radius:8px;border:none;color:white;font-weight:500;cursor:pointer;transition:all 0.2s;">Update Service</button>
        </div>
    </form>
</div>
